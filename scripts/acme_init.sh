...
if [ "${INIT_UPTIME_KUMA^^}" == "TRUE" ]; then
  ...
  bash scripts/acme_run.sh --issue --domain "${CERT_DOMAIN_1}" --domain "${CERT_DOMAIN_2}" --domain "${CERT_DOMAIN_3}" --dns "${CERT_DNS}" --server "${ACME_DIRECTORY_RESOURCE}" ${FORCE:-} \
...
if [ "${INIT_HOMEBRIDGE^^}" == "TRUE" ]; then
  ...
  bash scripts/acme_run.sh --issue --domain "${CERT_DOMAIN}" --dns "${CERT_DNS}" --server "${ACME_DIRECTORY_RESOURCE}" ${FORCE:-}
